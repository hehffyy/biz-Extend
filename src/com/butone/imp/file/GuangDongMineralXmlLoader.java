 ...
 *
 */
public class GuangDongMineralXmlLoader implements TableDataImport, TableDataExporter {
	...
	*/
		...
		if (Utils.isNotEmptyString(dkInfo)) {
			...
			while (dkCnt > 0) {
				...
				for (int idx = 1; idx <= pointCount; idx++) {
					//x、y坐标的先后顺序
					BigDecimal x = new BigDecimal(args[pos + idx * 2-1]);
					BigDecimal y = new BigDecimal(args[pos + idx * 2]);
	...
	public InputStream tableToStream(Map<String, Table> tables, Map<String, Object> variants) {
		try {
			...
			SAXReader reader = new SAXReader();
			Document doc = reader.read(this.getClass().getResourceAsStream("GuangDongXmlLoader.xml"));
			...
			if (zbTable != null && dkTable != null) {
				...
				doc.selectSingleNode("APP/DATA/NA_APPENDIX").setText(NA_AREA_COORDINATE);
			}
