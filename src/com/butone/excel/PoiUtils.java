...
public final class PoiUtils {
	...
	public static String getCellValue(Row row, String fldName) {
		Object value = row.getValue(fldName);
		if (value == null)
			return "";
		else {
	...
	public static void removeRegions(HSSFSheet sheet,int regionCount){
		for(int i=0;i<regionCount;i++){
			 sheet.removeMergedRegion(i);
		}
	}
	public static void delteRows(XSSFSheet sheet,int fromRow,int toRow){
...
//			sheet.removeRow(row);
//		}
		sheet.setDefaultRowHeight(sheet.getRow(fromRow).getHeight());
		sheet.shiftRows(fromRow+1,sheet.getLastRowNum()+1,fromRow-(toRow+1));
	}
     ...
     */
    public static void copyRows(XSSFWorkbook wb,int fromRow,int toRow,int targetRowIndex,int regions)throws Exception {
         
        //复制源表中的行
        int maxColumnNum = 0;
 
        Map styleMap = (true) ? new HashMap() : null;
         XSSFSheet sheet =wb.getSheetAt(0);
        XSSFDrawing patriarch = sheet.createDrawingPatriarch(); //用于复制注释
        for (int i = fromRow; i <toRow; i++) {
            XSSFRow sourceRow = sheet.getRow(i);
            XSSFRow targetRow = sheet.createRow(targetRowIndex+toRow-i);
             
            if (sourceRow != null) {
                copyRow(targetRow, sourceRow,
                		wb, wb,patriarch, styleMap);
      ...
      /*  for (int i = 0; i < regions; i++) {
            ...
            CellRangeAddress newRange = new CellRangeAddress(
                    oldRange.getFirstRow()+targetRowIndex, oldRange.getLastRow()+targetRowIndex,
...
//        }
     ...
     * @param targetPatriarch
     */
    public static void copyComment(XSSFCell targetCell,XSSFCell sourceCell,XSSFDrawing targetPatriarch)throws Exception{
